    ...
    <script type="text/javascript">
    	...
    	function setUpTime()
    	{
    		...
    		$.ajax({
				type: "get",
				async:false,
				url: "<%=basePath%>getPhoneBySendDate.action?phone=${cellnum}"+"&timeStamp" + new Date().getTime(),
				cache:false,
    	...
    	function getCode(){
    		if($("#cellphone").val()!="")
	    	{
    			...
    			$.ajax({
					...
					async:false,
					cache:false,
					url: '<%=basePath%>sendPhoneCode.action?phone=${cellnum}&timeStamp' + new Date().getTime(),
		...
		$(function () {
			 $("#testCode").click(function(){
    			...
    			else
    			{
    				$.ajax({
						type: "get",
						url: "<%=basePath%>safetycenter/bindmobile2.action?code="+confirmCode +"&timeStamp" + new Date().getTime(),
						cache:false,
						success: function(data, textStatus){
							if(data == "1"){
								location.href='<%=basePath %>safetycenter/bindmobile3.action';
							}else{
								document.getElementById("confirmCode").focus();
                    ...
                    <div class="form-psd btm-dh">
                     	<div class="item-psd">
                            ...
                            <div class="number-psd fl">
                            	<strong>${cellphone }</strong> <input type="hidden" id="cellphone" value="${cellnum }"/>
