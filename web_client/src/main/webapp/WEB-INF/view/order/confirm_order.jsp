	...
	<body> 
			...
			<div class="getorder-cont">
					...
					<div class="order-payment-type" id="edit_invoice_info">
						...
						<div style="margin-bottom: 15px;" class="none" id="invoice_address">
							...
							<c:if test="${invoice.invoice_top == '单位'}">
								<input checked="checked" type="radio" name="invoice_top" value="单位" >&nbsp; 单位&nbsp;&nbsp;</input>
				...
				<div class="order-payment" style="border-bottom:0">
					...
					<table border="0" cellpadding="0" class="order-list" cellspacing="1">
						...
						<tr>
							<td>
								<a href="<%=basePath%>goods/${sessionScope.buy_instantly.goods.goods_id}.action" title="${sessionScope.buy_instantly.goods.goods_name}"> <c:if test="${!empty sessionScope.buy_instantly.goods.img_path}">
										<img src="<h:imgSubstr imgpath="${sessionScope.buy_instantly.goods.img_path}"/>" onload="DrawImage(this,90,90)" />
									</c:if> ${sessionScope.buy_instantly.goods.goods_name}${sessionScope.buy_instantly.goods_param } </a>
							</td>
							<td>
								¥${sessionScope.buy_instantly.goods.sale_price}
							</td>
							<td>
								¥0.00
							</td>
							<td>
								<fmt:formatNumber type="number" pattern="¥0.00" value="${sessionScope.buy_instantly.goods.sale_price * sessionScope.buy_instantly.amount } " maxFractionDigits="2" />
							</td>
							<td>
								${sessionScope.buy_instantly.amount}
							</td>
							<td>
								有货
							</td>
		...
		<script>
			...
			function subOrder(){
				...
				location.href="<%=basePath%>order/confirm_order_ok.action";
			}
				 ...
				 if($('#addr_list input[@type=radio]:checked').length<1&& $("#add_address_info").is(":hidden")) {
				  ...
				  if(cust_name.length<=0){
					...
					if(address.length<=0){
						...
						$("#span_address").html("详细地址不能超过五十个字符！");
						$("#address").focus();
						return false;
					}else{
						$("#span_address").html("");
