...
<table width="100%" cellpadding="0" cellspacing="8" class="add_tab tr td">
	<tr>
		...
		<td width="85%">
			${catAttrName}<a href="<%=basePath %>goods/category.action?catAttr=${sellerGoodsWithBLOBs.catAttr}">[重新选择]</a>
			...
			<c:choose>
				<c:when test="${item.attrType == '0'}">
					...
					<input type="text"   name="goodsAttrList[${loop.index}].attrValueList[0].attrValue" class="w400"
						value="${sellerGoodsWithBLOBs.goodsAttrList[loop.index].attrValueList[0].attrValue == null ? item.attrValueList[0].attrValue : sellerGoodsWithBLOBs.goodsAttrList[loop.index].attrValueList[0].attrValue}">
				</c:when>
				<c:when test="${item.attrType == '2'}">
					...
					<input type="radio" id="tradeId_${loop.index}_${lo.index}" class="chkbox"
				...
				<c:when test="${item.attrType == '3'}">
					...
					<c:forEach items="${item.attrValueList}" var="it" varStatus="lo">
					<input type="checkbox" id="tradeId_${loop.index}_${lo.index}" class="chkbox"
						name="goodsAttrList[${loop.index}].attrValueList[0].tradeId" 
						value="${it.tradeId}"
	...
	<tr>
		...
		<td>
			<sf:input path="imgPath" id="imgInput" style="display:none;" cssClass="w400" />
