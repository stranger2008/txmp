...
<div class="position">个人资金提现申请</div>
...
<table width="100%" cellpadding="0" cellspacing="0" class="info_tab">
	...
	<tr class="tab_td">
		...
		<td>
			<c:if test="${item.status == '0' }">
				<a href="<%=basePath%>fundwithdraw/memberuser-audit.action?trade_id=${item.trade_id}">审核</a>
			</c:if>
			<c:if test="${item.status != '0' }">
				<a href="<%=basePath%>fundwithdraw/memberuser-view.action?trade_id=${item.trade_id}">查看详情</a>
			</c:if>
		</td>
	</tr>
