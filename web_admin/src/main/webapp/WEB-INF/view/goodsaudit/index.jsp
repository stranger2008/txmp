...
<sf:form action="goods-audit/index.action" id="queryForm" modelAttribute="goodsQueryForm">
	...
	<ul>
		...
		<li>
			...
			<sf:select path="info_state">
				...
				<sf:option value="2">审核不通过</sf:option>
			</sf:select>
		</li>
...
<table width="100%" cellpadding="0" cellspacing="0" class="info_tab">
	...
	<tr class="tab_td">
		...
		<td>
			<c:if test="${item.info_state == '0'}"><a href="<%=basePath%>goods-audit/update.action?id=${item.goods_id}">审核</a></c:if>
			<c:if test="${item.info_state == '2'}"><a href="<%=basePath%>goods-audit/view.action?id=${item.goods_id}">查看</a></c:if>
		</td>
	</tr>
