...
<sf:form action="member/index.action" id="queryForm" modelAttribute="memberQueryForm">
...
<div class="kjbutdiv">
	...
	<a href="<%=basePath%>member/add.action">新增</a>
...
<table width="100%" cellpadding="0" cellspacing="0" class="info_tab">
	...
	<tr class="tab_td">
		...
		<td>
			<a href="<%=basePath%>member/update.action?id=${item.cust_id}">修改</a>
			<a href="<%=basePath%>member/reset-passwd.action?cust_id=${item.cust_id}">重置密码</a>
			<a href="<%=basePath%>member/view.action?id=${item.cust_id}">企业信息</a>
			<a href="<%=basePath%>shopconfig/view.action?cust_id=${item.cust_id}">店铺信息</a>
			<a href="<%=basePath%>levelinfo/view.action?cust_id=${item.cust_id}">等级管理</a>
		</td>
	</tr>
