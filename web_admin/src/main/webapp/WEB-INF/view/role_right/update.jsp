...
function changeSysmenu(value) {
	...
	$.ajax({
		...
		data: {syscode: value, right_id: '${role_right.right_id}'},
		dataType: 'json',
		success: function(data) {
			...
			$.each(data,function(entryIndex,entry){
				var text = '<input type="radio" name="menu_attr" value="' + entry['menu_id'] + '" id="' + entry['menu_id'] + '" pid="' + entry['up_menu_id']+ '"';
				if(entry['has_right']) {
					text = text + ' checked="checked"';
				}
				text = text + ' />' + entry['menu_name'];
            	d.add(entry['menu_id'], entry['up_menu_id'], text);
             });
...
<div class="position">修改权限</div>

<sf:form method="post" action="role_right/update.action" modelAttribute="role_right">
