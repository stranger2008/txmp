...
import com.xingfugo.web.admin.module.Goodsaskreply;
...
public class GoodsaskController extends BaseController{
	...
	//商城后台咨询列表
	@RequestMapping(value="goodsask/index")
	public String goodsaskList(GoodsaskQueryForm goodsaskQueryForm,ModelMap model) throws Exception {
		PageResult pageResult = goodsaskService.getGoodsaskByPage(goodsaskQueryForm);
		pageOper(model, pageResult);
		model.addAttribute("goodsaskQueryForm", goodsaskQueryForm);
		return "goodsask/index";
	}
	
	//商城后台删除
	@RequestMapping(value="goodsask/delete",method=RequestMethod.POST)
	public String delete(String id,RedirectAttributes rAttr) throws Exception {
		goodsaskService.delete(id);
		operatePrompt(rAttr, "删除商品咨询成功");
		return "redirect:"+basePath()+"goodsask/index.action";
	}
