...
<head>
	...
	<body>
		<div class="head">
			<h2 id="navtit">购买七乐彩</h2>
				<a href="<%=basePath %>lottery/lotteryInfo.action?gameid=4" class="btn-qgkj" id="sourceUrl">
					<span><em></em></span>继续选号
				</a>
		</div>
	<form action="<%=basePath %>lottery/qlcsubmitorder.action" method="post">
		<input type="hidden" 	name ="ballList" 						id="ballList">
		<input type="hidden" 	name ="period" 							id="period"> 
	 	<input type="hidden" 	name="requesttype" 			value="4"  	id="requesttype">
	  	<input type="hidden"	name="gameid" 				value="4"  	id="gameid">
		...
		<div class="w320">
		  <div class="area1">
		    <div class="xq-tit" issue="2014084" issale="1" endtime="1405943820">
		      <h3>第<cite id="actQH"><script>document.write(window.sessionStorage.getItem('periodqlc'));</script></cite>期选号内容</h3>
		...
		<script>
			$("#currendtime").val(window.sessionStorage.getItem("currendtimeqlc"));
			 monitor.setProject('360_cp_m').getTrack().getClickAndKeydown();
			 $("#period").val(window.sessionStorage.getItem('periodqlc'));
		</script>
