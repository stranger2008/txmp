	...
	$(function (){
		...
		$(":checkbox[name='itemCheck']").each(function(i,v){
			...
			$(this).click(function (){
				...
				if($(this).attr("checked")){
					var cur_tp = parseFloat(cur_totalPrice)+price*num ;
					...
					$("#iptCheckbox").attr("checked",allchecked);
				}else{
					var cur_tp = parseFloat(cur_totalPrice)-price*num ;
					var cut_nm = parseInt(cur_totalNum)-num;
					$("#goodsTotalPrice_inp").val(cur_tp);
					$("#goodsTotalPrice").text(formatCurrency(cur_tp));
					$("#tatalGoods").text(cut_nm);
					$("#iptCheckbox").attr("checked",false)
				}
			});
			if($(this).attr("checked")){
					tempTotalMoney = tempTotalMoney + price*num ;
					tempTotalnum += num;
		...
		$("#iptCheckbox").click(function(){
			var ck = $(this).attr("checked");
			$(":checkbox[name='itemCheck']").attr("checked",ck);
			if(ck){
				var cur_tp = 0 ;
				var cut_nm = 0;
				$(":checkbox[name='itemCheck']").each(function(i,v){
