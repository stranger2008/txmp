...
function showSelectCascade(bp_temp,modelName,showDivIdName,fieldName,modelValueStr){
	selectCascadeUrl = bp_temp+"public/showSelectLink.action";
	showSelectDivModel(modelName,"1111111111",showDivIdName,fieldName,3,modelValueStr);
}
...
function showSelectDivModel(modelName,upModelId,showDivIdName,fieldName,totalNum,modelValueStr){
	var selectStr = getSelectStr(fieldName,totalNum,modelName);
	...
	$.ajax({
		type: "get",
		async:false,
		url: selectCascadeUrl+"?model_name="+modelName+"&up_model_id="+upModelId,
	...
	if(modelValueStr != ""){
		...
		for(var i=0;i<tnum;i++){
			...
			$.ajax({
				type: "get",
				async:false,
				url: selectCascadeUrl+"?model_name="+modelName+"&up_model_id="+_upModelId,
				success: function(data, textStatus){
					var k = i+1;
...
function setSelectValue(modelName,upModelId,fieldName,totalNum,thisNum){
	if(totalNum >= thisNum){
		$.ajax({
			type: "get",
			async:false,
...
function getSelectStr(fieldName,totalNum,modelName){
	var selectStr = "";
	for(var i=1;i<=totalNum;i++){
		selectStr += "<select name='select_"+fieldName+"' id='"+replacepoint(fieldName)+i+"' style='margin-right:3px;'  onchange=setSelectValue('"+modelName+"',this.value,'"+fieldName+"',"+totalNum+","+i+");  >";
